...
def extract_message_data(message: types.Message):
    ...
    """
    # Use caption if it's media with text, otherwise use message.text
    ...
    return f"[{message.content_type}]", message.content_type, None, None
...
async def process_message(message: types.Message, db):
    ...
    try:
        # 2. Echo behaviour (Using file_id to prevent re-uploading)
        sent_message = None
        if msg_type == "text":
        ...
        # Note: The sent_message will have a NEW file_id generated by Telegram, 
        # so we extract it from the result object to keep the DB perfectly accurate.
        if sent_message:
