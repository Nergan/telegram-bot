...
async def init_edit_media(message: types.Message, state: FSMContext, lang: str, profile_service: ProfileService):
    ...
    has_media = bool(active_prof.media)
...
async def capture_media(message: types.Message, state: FSMContext, lang: str, profile_service: ProfileService, album_service: AlbumService):
    if message.text and message.text in _btn("btn_cancel"):
        from bot.handlers.base import fsm_cancel
        await fsm_cancel(message, state, lang, profile_service)
        return
    ...
    media_doc = MediaItem(type=message.content_type)
    ...
    if message.media_group_id:
        ...
        media_list = await album_service.add_to_buffer(message.media_group_id, media_doc)
        if len(media_list) == 1:
            try:
                await message.answer(_("album_proc", lang))
                await asyncio.sleep(1.5)
                final_media = await album_service.get_and_clear_buffer(message.media_group_id)
                if final_media:
                    active_prof.media = final_media
                    await profile_service.update_profile(active_prof)
                    await album_service.mark_processed(message.media_group_id)
                    await state.clear()
                    await message.answer(_("album_saved", lang), reply_markup=edit_info_menu_kb(lang))
            except Exception as e:
                await album_service.clear_buffer(message.media_group_id)
                logger.error(f"Distributed album processing error: {e}")
    else:
        active_prof.media = [media_doc]
        await profile_service.update_profile(active_prof)
...
async def manage_prof_cb(
    ...
    await callback.message.answer(_("prof_managing", lang), reply_markup=manage_action_kb(lang, profile.is_active))
...
async def manage_actions(
    ...
    if message.text == _("btn_set_active", lang):
        ...
        p.public_uuid = new_uuid
        await profile_service.update_profile(p)
        await state.update_data(managing_uuid=new_uuid)
        
        await message.answer(_("prof_regen", lang), reply_markup=manage_action_kb(lang, p.is_active))
